...
Run `python3 samples/benchmark/patterns.py` to print every (id, formula) it emits.
"""
from __future__ import annotations

from typing import Iterator, List, Tuple

from survey.normalize import normalize_ltl
...
def emit(root: str, lengths: range = range(2, 5)) -> List[str]:
    ...
    for name, (title, items) in groups.items():
        ...
        with open(path, "w", encoding="utf-8") as fh:
            fh.write(f"# {title}\n"
                     f"# generated by samples/benchmark/patterns.py, AP-renamed to "
                     f"canonical a,b,c… -- do not hand-edit\n\n")
            for pid, f in items:
                fh.write(f"{normalize_ltl(f)}  # {pid}\n")
        written.append(path)
    return written
