"""
buchi2ltl/reconstruction_dag.py — DAG-native port of reconstruct_ltl.

TEMPORARY parallel implementation, ported PARAGRAPH-BY-PARAGRAPH from the string
engine `reconstruction.reconstruct_ltl`: same control flow, same case splits,
same comments — but every formula is built as a hash-consed `spot.formula`
instead of a string. Consequences:
  * an adopted scc_labeler formula (a kr label) is spliced WITHOUT flattening
    (the whole point — str() on a high-sharing kr DAG is the explosion);
  * the output is a formula DAG end to end (no final string re-parse / cleanups).

Kept BESIDE the string engine so the two can be CROSS-ORACLED
(`kr/testing/probe_dag_oracle.py`: run both, check are_equivalent) until this is
stable; then the string engine is deleted. Pure helpers are SHARED (imported).
The UNSUPPORTED decline is the same string sentinel (discriminated by
isinstance(str)); on success the return is a `spot.formula`.
"""
...
def reconstruct_ltl_dag(aut, scc_labeler=None):
    """DAG-native backward reconstruction. Returns (final_formula, state_formula,
    technique); final_formula is a spot.formula on success, or the UNSUPPORTED
    string on decline. See module docstring."""
    ...
    # As FORMULAS (None when empty) — the only deviation from the string engine,
    # which stored "G(...)" strings.
    ...
    for info in nice_terminal_sccs:
        ...
        if validated:
            for st in info["states"]:
                # t2 now emits a spot.formula DAG (see terminal_2scc); use directly.
