...
- **Muller subset lift (elevated from P1; minimal failing case GFa).** `compute_good_muller_sets` (cascade.py) only emits whole non-rejecting SCCs of the pruned config aut. The correct Muller alpha' includes every reachable M that induces a strongly connected subgraph (singleton needs self-loop) and is accepting. Plan: for each *non-rejecting* SCC (rejecting SCCs cannot contain accepting sub-cycles — prunes the powerset), enumerate subsets, check induced strong connectivity, then `g.acc().accepting(union of marks on in-M edges)` as the general acceptance oracle (sound under sbacc since marks are state-consistent). Gate by SCC size (env-tunable), log truncation to whole-SCC (no silent caps). Spot --decompose-scc=aN gives only basins (already a fallback), not sub-cycles, so this is implemented in-repo. Acceptance tests: GFa, FGa, G(a->Fb) equiv True.
